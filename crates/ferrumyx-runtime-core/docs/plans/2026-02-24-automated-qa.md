# Automated QA Plan for Ferrumyx Runtime Core
...
A review of all closed issues and merged bug-fix PRs reveals that most Ferrumyx Runtime Core bugs fall into a few recurring categories:
...
Add the same validation for WASM tools (loaded from `~/.ferrumyx-runtime-core/tools/`) and MCP tools (mock a simple MCP manifest and validate the schema it produces).
...
docker-build:
  ...
  steps:
    ...
    - name: Build Docker image
      run: docker build -t ferrumyx-runtime-core-test:ci .
```
...
**Cost:** High (requires Anthropic computer use API, headless browser, ferrumyx-runtime-core running)
...
```
+------------------+     +-----------------+     +------------------+
|  Test Runner     |     |  Headless       |     |  Ferrumyx Runtime Core        |
...
1. **Test runner** -- Python or TypeScript script that orchestrates the flow. Starts ferrumyx-runtime-core, waits for readiness, launches Playwright browser, runs scenarios.
...
tests/
  e2e/
    conftest.py             # pytest fixtures: start ferrumyx-runtime-core, browser
    ...
    Dockerfile.test         # Container for CI: ferrumyx-runtime-core + chromium
```

**Fixture: start ferrumyx-runtime-core**

```python
@pytest.fixture(scope="session")
async def ferrumyx-runtime-core_server():
    """Start ferrumyx-runtime-core with gateway enabled, return base URL."""
    env = {
        "CLI_ENABLED": "false",
...
```python
@pytest.fixture
async def browser_agent(ferrumyx-runtime-core_server):
    """Playwright browser + Claude computer use agent."""
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page(viewport={"width": 1280, "height": 720})
        await page.goto(f"{ferrumyx-runtime-core_server}/?token=test-token-e2e")
...
```python
async def test_sse_reconnect_preserves_history(browser_agent, ferrumyx-runtime-core_server):
    ...
    await restart_ferrumyx-runtime-core(ferrumyx-runtime-core_server)
...
```python
async def test_onboarding_wizard_completes(tmp_ferrumyx-runtime-core_home):
    """Bugs: #187, #174, #129, #185 (wizard persistence and re-trigger)"""
    # Start ferrumyx-runtime-core with a fresh home directory (no prior config)
    ...
    proc = pexpect.spawn(
        "cargo run",
        env={"FERRUMYX_RUNTIME_CORE_HOME": str(tmp_ferrumyx-runtime-core_home), **base_env},
        timeout=60,
    )

    # Step through wizard
    proc.expect("Welcome to Ferrumyx Runtime Core")
    ...
    proc2.expect("Agent ferrumyx-runtime-core ready")  # Should skip wizard
    # Must NOT see "Welcome to Ferrumyx Runtime Core" again
    assert not proc2.match_any(["Welcome to Ferrumyx Runtime Core"], timeout=5)
    proc2.close()
```
...
jobs:
  e2e:
    ...
    steps:
      - uses: actions/checkout@v6
      - name: Build ferrumyx-runtime-core
