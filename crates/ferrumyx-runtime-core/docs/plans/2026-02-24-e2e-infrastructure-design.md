...
**Date:** 2026-02-24
**Status:** Approved
**Goal:** Deterministic browser-level E2E tests for the Ferrumyx Runtime Core web gateway using Python + Playwright, with a mock LLM backend for CI reliability.
...
```
         ...
         +----------+-----------+
         |                      |
   mock_llm.py           ferrumyx-runtime-core binary
...
2. Session-scoped fixture builds ferrumyx-runtime-core binary (or reuses cached)
3. Session-scoped fixture starts mock LLM on OS-assigned port
4. Session-scoped fixture starts ferrumyx-runtime-core subprocess pointing to mock LLM, gateway on OS-assigned port, libSQL in-memory
...
7. Teardown kills ferrumyx-runtime-core and mock LLM
...
```
tests/e2e/
  conftest.py              # pytest fixtures: build binary, start ferrumyx-runtime-core, mock LLM, browser
...
- Supports `stream: true` by returning SSE chunks with `delta` objects (critical: Ferrumyx Runtime Core streams responses via SSE to the browser)
...
- Checks if `./target/debug/ferrumyx-runtime-core` exists
...
**`ferrumyx-runtime-core_server(ferrumyx-runtime-core_binary, mock_llm_server)`**
- Starts the ferrumyx-runtime-core binary with environment:
...
- Parses actual gateway port from ferrumyx-runtime-core stdout (`Gateway listening on 127.0.0.1:XXXX`)
...
**`page(ferrumyx-runtime-core_server)`**
...
Ferrumyx Runtime Core logs `Gateway listening on 127.0.0.1:XXXX` at startup. The fixture reads stdout line-by-line until it finds this pattern, extracts the port.
...
async def wait_for_port(process, pattern=r"Gateway listening on .+:(\d+)", timeout=60):
    ...
    raise TimeoutError("ferrumyx-runtime-core did not report listening port")
```
...
1. `pytest tests/e2e/ -v` passes locally with a pre-built ferrumyx-runtime-core binary
