...
**Goal:** Build a Python + Playwright E2E testing framework that exercises the Ferrumyx Runtime Core web gateway through a real browser against the real binary with a mock LLM backend.

**Architecture:** pytest session fixtures start a mock OpenAI-compat HTTP server and the ferrumyx-runtime-core binary (libSQL in-memory, gateway enabled), then per-test Playwright browser instances navigate to the gateway and make DOM assertions.
...
```toml
[project]
name = "ferrumyx-runtime-core-e2e"
version = "0.1.0"
...
- Support `stream: true` with proper SSE chunk format (critical for Ferrumyx Runtime Core's streaming)
...
- Ferrumyx Runtime Core logs `Web UI: http://{host}:{port}/` to stdout (main.rs:508) using the config port, not the bound port. So we must use a fixed port, not port 0.
...
Session-scoped: build binary, start mock LLM, start ferrumyx-runtime-core.
...
def ferrumyx-runtime-core_binary():
    """Ensure ferrumyx-runtime-core binary is built. Returns the binary path."""
    binary = ROOT / "target" / "debug" / "ferrumyx-runtime-core"
    if not binary.exists():
        print("Building ferrumyx-runtime-core (this may take a while)...")
        subprocess.run(
...
@pytest.fixture(scope="session")
async def ferrumyx-runtime-core_server(ferrumyx-runtime-core_binary, mock_llm_server):
    """Start the ferrumyx-runtime-core gateway. Yields the base URL."""
    env = {
        **os.environ,
        "RUST_LOG": "ferrumyx-runtime-core=info",
...
@pytest.fixture
async def page(ferrumyx-runtime-core_server):
    ...
    async with async_playwright() as p:
        ...
        await pg.goto(f"{ferrumyx-runtime-core_server}/?token={AUTH_TOKEN}")
...
git commit -m "feat: E2E conftest with session fixtures for mock LLM and ferrumyx-runtime-core"
```
...
async def test_auth_rejection(page, ferrumyx-runtime-core_server):
    """Navigating without a token shows the auth screen."""
    # Open a new page without the token
    new_page = await page.context.new_page()
    await new_page.goto(ferrumyx-runtime-core_server)
    auth_screen = new_page.locator(SEL["auth_screen"])
    await auth_screen.wait_for(state="visible", timeout=10000)
    await new_page.close()
```

**Step 2: Verify test runs (may fail if ferrumyx-runtime-core isn't built yet -- that's OK)**

```bash
cd tests/e2e && python -m pytest scenarios/test_connection.py -v --timeout=120
```

Expected: Tests pass if ferrumyx-runtime-core is built, or skip/fail gracefully if not.
...
jobs:
  e2e:
    ...
    steps:
      ...
      - name: Build ferrumyx-runtime-core (libsql)
...
Browser-level end-to-end tests for the Ferrumyx Runtime Core web gateway using Python + Playwright.

## Prerequisites

- Python 3.11+
- Rust toolchain (for building ferrumyx-runtime-core)
...
## Build ferrumyx-runtime-core

The tests need the ferrumyx-runtime-core binary built with libsql support:
...
2. **Ferrumyx Runtime Core** -- the real binary with gateway enabled, pointing to the mock LLM
...
**Step 1: Build ferrumyx-runtime-core**
