...
This guide covers configuring the Telegram channel for Ferrumyx Runtime Core, including DM pairing for access control.

## Overview

The Telegram channel lets you interact with Ferrumyx Runtime Core via Telegram DMs and groups. It supports:
...
- Ferrumyx Runtime Core installed and configured (`ferrumyx-runtime-core onboard`)
...
2. Bot replies: `To pair with this bot, run: ferrumyx-runtime-core pairing approve telegram ABC12345`
3. You run: `ferrumyx-runtime-core pairing approve telegram ABC12345`
...
ferrumyx-runtime-core pairing list telegram

# List as JSON
ferrumyx-runtime-core pairing list telegram --json

# Approve a user by code
ferrumyx-runtime-core pairing approve telegram ABC12345
```

### Configuration

Edit `~/.ferrumyx-runtime-core/channels/telegram.capabilities.json` (or the config injected by the host):
...
# Install
mkdir -p ~/.ferrumyx-runtime-core/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.ferrumyx-runtime-core/channels/
```
...
- **Secrets store**: `ferrumyx-runtime-core` CLI (if available)
...
- Set `bot_username` in config to your bot's username (e.g., `MyFerrumyx Runtime CoreBot`)
- Ensure the message contains `@YourBot` or starts with `/`

### "Connection refused" when starting

- For webhook mode: Start your tunnel before `ferrumyx-runtime-core run`
