...
Ferrumyx Runtime Core defaults to NEAR AI for model access, but supports any OpenAI-compatible
...
No additional configuration required. On first run, `ferrumyx-runtime-core onboard` opens a browser
for OAuth authentication. Credentials are saved to `~/.ferrumyx-runtime-core/session.json`.
