...
This guide covers how to build WASM channel modules for Ferrumyx Runtime Core.
...
```
~/.ferrumyx-runtime-core/channels/
├── my-channel.wasm
└── my-channel.capabilities.json
```
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for Ferrumyx Runtime Core"
...
**Do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. Ferrumyx Runtime Core builds channels from source:
...
# Install (or use ferrumyx-runtime-core onboard to install bundled channel)
...
**Note**: The main Ferrumyx Runtime Core binary bundles `telegram.wasm` via `include_bytes!`. When modifying the Telegram channel source, run `./channels-src/telegram/build.sh` **before** building the main crate, so the updated WASM is included.
...
# Deploy to ~/.ferrumyx-runtime-core/channels/
cp target/wasm32-wasip2/release/my_channel.wasm ~/.ferrumyx-runtime-core/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.ferrumyx-runtime-core/channels/
```
