...
|----------|----------|---------|-------------|
| `FERRUMYX_RUNTIME_CORE_RECORD_TRACE` | yes | — | Set to any non-empty value to enable recording. |
| `FERRUMYX_RUNTIME_CORE_TRACE_OUTPUT` | no | `./trace_{timestamp}.json` | Output file path for the recorded trace. |
| `FERRUMYX_RUNTIME_CORE_TRACE_MODEL_NAME` | no | `recorded-{model}` | The `model_name` field in the trace JSON. |

### Usage

```bash
# Record a trace (writes to ./trace_20260304T120000.json)
FERRUMYX_RUNTIME_CORE_RECORD_TRACE=1 cargo run

# Custom output path
FERRUMYX_RUNTIME_CORE_RECORD_TRACE=1 FERRUMYX_RUNTIME_CORE_TRACE_OUTPUT=my_trace.json cargo run

# Custom model name
FERRUMYX_RUNTIME_CORE_RECORD_TRACE=1 FERRUMYX_RUNTIME_CORE_TRACE_MODEL_NAME=regression-auth-flow cargo run
```
