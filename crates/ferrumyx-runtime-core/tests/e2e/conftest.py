...
Session-scoped: build binary, start mock LLM, start ferrumyx-runtime-core, launch browser.
...
_DB_TMPDIR = tempfile.TemporaryDirectory(prefix="ferrumyx-runtime-core-e2e-")
...
async def ferrumyx-runtime-core_server(ferrumyx-runtime-core_binary, mock_llm_server):
    ...
    for key, val in os.environ.items():
        if key.startswith(COV_ENV_PREFIXES) or key in COV_ENV_EXTRAS:
            env[key] = val
    proc = await asyncio.create_subprocess_exec(
        ferrumyx-runtime-core_binary, "--no-onboard",
    ...
    try:
        ...
        proc.kill()
        pytest.fail(
            f"ferrumyx-runtime-core server failed to start on port {gateway_port} "
...
@pytest.fixture(scope="session")
async def browser(ferrumyx-runtime-core_server):
...
@pytest.fixture
async def page(ferrumyx-runtime-core_server, browser):
