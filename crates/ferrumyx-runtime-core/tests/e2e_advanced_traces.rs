...
mod advanced {
    ...
    async fn user_steering() {
        let _cleanup = CleanupGuard::new().file("/tmp/runtime_core_steer_test.txt");
        let _ = std::fs::remove_file("/tmp/runtime_core_steer_test.txt");
        ...
        let content = std::fs::read_to_string("/tmp/runtime_core_steer_test.txt")
    ...
    async fn tool_error_recovery() {
        let _cleanup = CleanupGuard::new().file("/tmp/runtime_core_recovery_test.txt");
        let _ = std::fs::remove_file("/tmp/runtime_core_recovery_test.txt");
        ...
        let content = std::fs::read_to_string("/tmp/runtime_core_recovery_test.txt")
    ...
    async fn long_tool_chain() {
        let test_dir = "/tmp/runtime_core_chain_test";
        ...
        rig.send_message(
            "Create a daily log at /tmp/runtime_core_chain_test/log.md, \
    ...
    async fn routine_news_digest() {
        use ferrumyx_runtime_core::llm::recording::{HttpExchange, HttpExchangeRequest, HttpExchangeResponse};
