...
use ferrumyx_runtime_core::channels::web::server::{GatewayState, start_server};
use ferrumyx_runtime_core::channels::web::sse::SseManager;
use ferrumyx_runtime_core::channels::web::ws::WsConnectionTracker;
use ferrumyx_runtime_core::error::LlmError;
use ferrumyx_runtime_core::llm::{
...
impl LlmProvider for MockLlmProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        let user_msg = req
            ...
            .iter()
            .rev()
            .find(|m| m.role == ferrumyx_runtime_core::llm::Role::User)
    ...
    async fn complete_with_tools(
        ...
        if let Some(tool) = req.tools.first() {
            Ok(ToolCompletionResponse {
                content: None,
                tool_calls: vec![ferrumyx_runtime_core::llm::ToolCall {
...
async fn start_test_server_with_provider(
    ...
    let state = Arc::new(GatewayState {
        ...
        skill_registry: None,
        skill_catalog: None,
        chat_rate_limiter: ferrumyx_runtime_core::channels::web::server::RateLimiter::new(30, 60),
...
async fn test_chat_completions_streaming() {
    ...
    assert_eq!(
        resp.headers()
            .get("x-ferrumyx_runtime_core-streaming")
            .and_then(|v| v.to_str().ok()),
        Some("simulated"),
        "Expected x-ferrumyx_runtime_core-streaming: simulated header"
    );
