...
use ferrumyx_runtime_core::agent::{Agent, AgentDeps};
use ferrumyx_runtime_core::app::{AppBuilder, AppBuilderFlags};
use ferrumyx_runtime_core::channels::web::log_layer::LogBroadcaster;
use ferrumyx_runtime_core::channels::{Channel, IncomingMessage, MessageStream, OutgoingResponse, StatusUpdate};
use ferrumyx_runtime_core::config::Config;
use ferrumyx_runtime_core::db::Database;
use ferrumyx_runtime_core::error::ChannelError;
use ferrumyx_runtime_core::llm::{LlmProvider, SessionConfig, SessionManager};
use ferrumyx_runtime_core::tools::Tool;
...
use ferrumyx_runtime_core::llm::recording::{HttpExchange, ReplayingHttpInterceptor};
...
pub struct TestRig {
    ...
    #[cfg(feature = "libsql")]
    workspace: Option<Arc<ferrumyx_runtime_core::workspace::Workspace>>,
...
impl TestRig {
    ...
    pub async fn send_incoming(&self, msg: ferrumyx_runtime_core::channels::IncomingMessage) {
    ...
    pub fn captured_llm_requests(&self) -> Vec<Vec<ferrumyx_runtime_core::llm::ChatMessage>> {
        self.trace_llm
            .as_ref()
...
impl TestRigBuilder {
    ...
    pub async fn build(self) -> TestRig {
        use ferrumyx_runtime_core::channels::ChannelManager;
        use ferrumyx_runtime_core::db::libsql::LibSqlBackend;
        ...
        let db: Arc<dyn ferrumyx_runtime_core::db::Database> = Arc::new(backend);
        ...
        // 6. Register job tools, routine tools, and extra tools.
        {
            ...
            if let (Some(db_arc), Some(ws)) = (&components.db, &components.workspace) {
                use ferrumyx_runtime_core::agent::routine_engine::RoutineEngine;
                use ferrumyx_runtime_core::config::RoutineConfig;
            ...
            http_interceptor: {
                ...
                if exchanges.is_empty() {
                    ...
                    Some(Arc::new(ReplayingHttpInterceptor::new(exchanges))
                        as Arc<dyn ferrumyx_runtime_core::llm::recording::HttpInterceptor>)
                }
            },
            transcription: None,
        ...
        let routine_config = if enable_routines {
            Some(ferrumyx_runtime_core::config::RoutineConfig {
                enabled: true,
...
impl TestRig {
    ...
    #[cfg(feature = "libsql")]
    pub fn workspace(&self) -> Option<&Arc<ferrumyx_runtime_core::workspace::Workspace>> {
