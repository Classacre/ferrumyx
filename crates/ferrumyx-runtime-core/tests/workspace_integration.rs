...
//! Set DATABASE_URL=postgres://localhost/runtime_core_test

use std::sync::Arc;

use ferrumyx_runtime_core::workspace::{MockEmbeddings, SearchConfig, Workspace, paths};

fn get_pool() -> deadpool_postgres::Pool {
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://localhost/runtime_core_test".to_string());
