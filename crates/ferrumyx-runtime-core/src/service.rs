//! OS service management for running Ferrumyx Runtime Core as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.ferrumyx_runtime_core.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/ferrumyx_runtime_core.service`
//!
//! The installed service runs `ferrumyx-runtime-core run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.ferrumyx_runtime_core.daemon";
const SYSTEMD_UNIT: &str = "ferrumyx_runtime_core.service";
...
fn install_macos() -> Result<()> {
    ...
    let logs_dir = runtime_core_logs_dir();
...
"#,
    ...
    println!("  Start with: ferrumyx-runtime-core service start");
    Ok(())
}
...
fn install_linux() -> Result<()> {
    ...
    let unit = format!(
        "[Unit]\n\
         Description=Ferrumyx Runtime Core daemon\n\
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `ferrumyx-runtime-core service install` first.");
        }
...
fn runtime_core_logs_dir() -> PathBuf {
    runtime_core_base_dir().join("logs")
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.ferrumyx_runtime_core.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/ferrumyx_runtime_core.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_runtime_core() {
        let path = runtime_core_logs_dir();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".ferrumyx_runtime_core/logs"), "unexpected path: {s}");
    }
}
