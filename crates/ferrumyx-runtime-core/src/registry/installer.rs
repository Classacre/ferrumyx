...
pub struct RegistryInstaller {
    ...
    repo_root: PathBuf,
    /// Directory for installed tools (`~/.ferrumyx_runtime_core/tools/`).
    tools_dir: PathBuf,
    /// Directory for installed channels (`~/.ferrumyx_runtime_core/channels/`).
...
impl RegistryInstaller {
    ...
    pub fn with_defaults(repo_root: PathBuf) -> Self {
        let base_dir = runtime_core_base_dir();
        Self {
            repo_root,
    ...
    pub async fn install_from_source(
        ...
        // CLI commands (`ferrumyx-runtime-core tool auth <name>`) all agree on the stem.
    ...
    pub async fn install_bundle(
        ...
        if let Some(shared) = &bundle.shared_auth {
            auth_hints.push(format!(
                "Bundle uses shared auth '{}'. Run `ferrumyx-runtime-core tool auth <any-member>` to authenticate all members.",
                shared
            ));
        }
...
mod tests {
    ...
    fn test_installer_creation() {
        let installer = RegistryInstaller::new(
            PathBuf::from("/repo"),
            PathBuf::from("/home/.ferrumyx_runtime_core/tools"),
            PathBuf::from("/home/.ferrumyx_runtime_core/channels"),
        );
    ...
    async fn test_install_from_artifact_rejects_non_https_url() {
        ...
        let manifest = test_manifest(
            ...
            "tools-src/demo",
            Some(
                "http://github.com/nearai/ferrumyx_runtime_core/releases/latest/download/demo.wasm".to_string(),
            ),
            None,
        );
    ...
    async fn test_install_from_artifact_rejects_null_sha256() {
        ...
        let manifest = test_manifest(
            ...
            "tools-src/demo",
            Some(
                "https://github.com/nearai/ferrumyx_runtime_core/releases/latest/download/demo-wasm32-wasip2.tar.gz".to_string(),
            ),
            None, // sha256 = null
        );
    ...
    fn test_should_attempt_source_fallback_policy() {
        let download = RegistryError::DownloadFailed {
            url: "https://github.com/nearai/ferrumyx_runtime_core/releases/latest/download/demo.wasm"
                .to_string(),
