...
pub struct DatabaseConfig {
    ...
    // -- libSQL fields --
    /// Path to local libSQL database file (default: ~/.ferrumyx_runtime_core/ferrumyx_runtime_core.db).
...
impl DatabaseConfig {
    pub(crate) fn resolve() -> Result<Self, ConfigError> {
        ...
        // DATABASE_URL is loaded from ~/.ferrumyx_runtime_core/.env via dotenvy early in startup.
        let url = optional_env("DATABASE_URL")?
            ...
            .ok_or_else(|| ConfigError::MissingRequired {
                ...
                hint: "Run 'ferrumyx-runtime-core onboard' or set DATABASE_URL environment variable".to_string(),
            })?;
...
/// Default libSQL database path (~/.ferrumyx_runtime_core/ferrumyx_runtime_core.db).
pub fn default_libsql_path() -> PathBuf {
    runtime_core_base_dir().join("ferrumyx_runtime_core.db")
}
