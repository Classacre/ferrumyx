//! Configuration for Ferrumyx Runtime Core.
//!
//! Settings are loaded with priority: env var > database > default.
//! `DATABASE_URL` lives in `~/.ferrumyx_runtime_core/.env` (loaded via dotenvy early
...
impl Config {
    ...
    pub fn for_testing(
        ...
        Self {
            ...
            channels: ChannelsConfig {
                ...
                gateway: None,
                signal: None,
                wasm_channels_dir: std::env::temp_dir().join("ferrumyx_runtime_core-test-channels"),
    ...
    pub async fn from_db_with_toml(
        ...
        crate::bootstrap::load_runtime_core_env();
    ...
    ///
    /// Loads both `./.env` (standard, higher priority) and `~/.ferrumyx_runtime_core/.env`
    ...
    /// If `None`, tries the default path `~/.ferrumyx_runtime_core/config.toml` (missing
