    ...
    pub(super) async fn handle_system_command(
        ...
        match command {
            ...
            "restart" => {
                ...
                tracing::debug!("[commands::restart] FERRUMYX_RUNTIME_CORE_IN_DOCKER={}", in_docker);
                ...
                if !in_docker {
                    ...
                    return Ok(SubmissionResult::error(
                        "Restart is not available in this environment. \
                         The FERRUMYX_RUNTIME_CORE_IN_DOCKER environment variable must be set to 'true' for Docker deployments."
                            .to_string(),
                    ));
                }
