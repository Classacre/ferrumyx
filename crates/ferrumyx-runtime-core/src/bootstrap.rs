//! Bootstrap helpers for Ferrumyx Runtime Core.
//!
...
//!
//! File: `~/.ferrumyx_runtime_core/.env` (standard dotenvy format)

use std::path::PathBuf;
use std::sync::LazyLock;

const FERRUMYX_RUNTIME_CORE_BASE_DIR_ENV: &str = "FERRUMYX_RUNTIME_CORE_BASE_DIR";

/// Lazily computed Ferrumyx Runtime Core base directory, cached for the lifetime of the process.
static FERRUMYX_RUNTIME_CORE_BASE_DIR: LazyLock<PathBuf> = LazyLock::new(compute_runtime_core_base_dir);

/// Compute the Ferrumyx Runtime Core base directory from environment.
///
/// This is the underlying implementation used by both the public
/// `runtime_core_base_dir()` function (which caches the result) and tests
/// (which need to verify different configurations).
pub fn compute_runtime_core_base_dir() -> PathBuf {
    std::env::var(FERRUMYX_RUNTIME_CORE_BASE_DIR_ENV)
        .map(PathBuf::from)
        .map(|path| {
            if path.as_os_str().is_empty() {
                ...
                eprintln!(
                    "Warning: FERRUMYX_RUNTIME_CORE_BASE_DIR is a relative path '{}', resolved against current directory",
                    path.display()
                );
                path
            } else {
...
/// Get the default Ferrumyx Runtime Core base directory (~/.ferrumyx_runtime_core).
///
/// Logs a warning if the home directory cannot be determined and falls back to
/// the current directory.
fn default_base_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".ferrumyx_runtime_core")
    } else {
        eprintln!("Warning: Could not determine home directory, using current directory");
        std::env::current_dir()
            .unwrap_or_else(|_| PathBuf::from("/tmp"))
            .join(".ferrumyx_runtime_core")
    }
}

/// Get the Ferrumyx Runtime Core base directory.
///
/// Override with `FERRUMYX_RUNTIME_CORE_BASE_DIR` environment variable.
/// Defaults to `~/.ferrumyx_runtime_core` (or `./.ferrumyx_runtime_core` if home directory cannot be determined).
///
/// Thread-safe: the value is computed once and cached in a `LazyLock`.
///
/// # Environment Variable Behavior
/// - If `FERRUMYX_RUNTIME_CORE_BASE_DIR` is set to a non-empty path, that path is used.
/// - If `FERRUMYX_RUNTIME_CORE_BASE_DIR` is set to an empty string, it is treated as unset.
/// - If `FERRUMYX_RUNTIME_CORE_BASE_DIR` contains null bytes, a warning is printed and the default is used.
...
/// for existence.
pub fn runtime_core_base_dir() -> PathBuf {
    FERRUMYX_RUNTIME_CORE_BASE_DIR.clone()
}

/// Path to the Ferrumyx Runtime Core-specific `.env` file: `~/.ferrumyx_runtime_core/.env`.
pub fn runtime_core_env_path() -> PathBuf {
    runtime_core_base_dir().join(".env")
}

/// Load env vars from `~/.ferrumyx_runtime_core/.env` (in addition to the standard `.env`).
///
/// Call this **after** `dotenvy::dotenv()` so that the standard `./.env`
/// takes priority over `~/.ferrumyx_runtime_core/.env`. dotenvy never overwrites
/// existing env vars, so the effective priority is:
///
///   explicit env vars > `./.env` > `~/.ferrumyx_runtime_core/.env` > auto-detect
///
/// If `~/.ferrumyx_runtime_core/.env` doesn't exist but the legacy `bootstrap.json` does,
...
/// `DATABASE_BACKEND` is still unset and `~/.ferrumyx_runtime_core/ferrumyx_runtime_core.db` exists,
/// defaults to `libsql` so cloud instances work out of the box without any
/// manual configuration.
pub fn load_runtime_core_env() {
    let path = runtime_core_env_path();
    ...
    // DATABASE_URL is configured but ferrumyx_runtime_core.db is already present.
    if std::env::var("DATABASE_BACKEND").is_err() {
        let default_db = dirs::home_dir()
            .unwrap_or_default()
            .join(".ferrumyx_runtime_core")
            .join("ferrumyx_runtime_core.db");
        if default_db.exists() {
            // SAFETY: `load_runtime_core_env` is called from a synchronous `fn main()`
...
fn migrate_bootstrap_json_to_env(env_path: &std::path::Path) {
    let runtime_core_dir = env_path
        .parent()
        .unwrap_or_else(|| std::path::Path::new("."));
    let bootstrap_path = runtime_core_dir.join("bootstrap.json");
...
/// Write database bootstrap vars to `~/.ferrumyx_runtime_core/.env`.
///
...
pub fn save_bootstrap_env(vars: &[(&str, &str)]) -> std::io::Result<()> {
    save_bootstrap_env_to(&runtime_core_env_path(), vars)
}
...
/// Update or add a single variable in `~/.ferrumyx_runtime_core/.env`, preserving existing content.
///
...
pub fn upsert_bootstrap_var(key: &str, value: &str) -> std::io::Result<()> {
    upsert_bootstrap_var_to(&runtime_core_env_path(), key, value)
}
...
/// Write `DATABASE_URL` to `~/.ferrumyx_runtime_core/.env`.
///
...
/// One-time migration of legacy `~/.ferrumyx_runtime_core/settings.json` into the database.
///
...
pub async fn migrate_disk_to_db(
    ...
    let runtime_core_dir = runtime_core_base_dir();
    let legacy_settings_path = runtime_core_dir.join("settings.json");
    ...
    // 2. Write DATABASE_URL to ~/.ferrumyx_runtime_core/.env
    if let Some(ref url) = settings.database_url {
        save_database_url(url)
            .map_err(|e| MigrationError::Io(format!("Failed to write .env: {}", e)))?;
        tracing::info!("Wrote DATABASE_URL to {}", runtime_core_env_path().display());
    }

    // 3. Migrate mcp-servers.json if it exists
    let mcp_path = runtime_core_dir.join("mcp-servers.json");
    if mcp_path.exists() {
    ...
    let session_path = runtime_core_dir.join("session.json");
    ...
    let old_bootstrap = runtime_core_dir.join("bootstrap.json");
...
/// Path to the PID lock file: `~/.ferrumyx_runtime_core/ferrumyx_runtime_core.pid`.
pub fn pid_lock_path() -> PathBuf {
    runtime_core_base_dir().join("ferrumyx_runtime_core.pid")
}

/// A PID-based lock that prevents multiple Ferrumyx Runtime Core instances from running
/// simultaneously.
///
...
pub enum PidLockError {
    #[error("Another Ferrumyx Runtime Core instance is already running (PID {pid})")]
...
mod tests {
    ...
    fn test_save_and_load_database_url() {
        ...
        let url = "postgres://localhost:5432/runtime_core_test";
        ...
        assert_eq!(
            content,
            "DATABASE_URL=\"postgres://localhost:5432/runtime_core_test\"\n"
        );
    ...
    fn test_save_database_url_with_hash_in_password() {
        ...
        let url = "postgres://user:p%23ss@localhost:5432/ferrumyx_runtime_core";
        std::fs::write(&env_path, format!("DATABASE_URL=\"{}\"\n", url)).unwrap();
...
INJECTED="pwned"#;
    ...
    #[test]
    fn test_runtime_core_env_path() {
        let path = runtime_core_env_path();
        assert!(path.ends_with(".ferrumyx_runtime_core/.env"));
    }
    ...
    fn test_migrate_bootstrap_json_to_env() {
        ...
        let bootstrap_json = serde_json::json!({
            "database_url": "postgres://localhost/runtime_core_upgrade",
    ...
    fn test_save_bootstrap_env_multiple_vars() {
        ...
        let vars = [
            ...
            ("LIBSQL_PATH", "/home/user/.ferrumyx_runtime_core/ferrumyx_runtime_core.db"),
        ];
        ...
        assert_eq!(
            ...
            (
                ...
                "/home/user/.ferrumyx_runtime_core/ferrumyx_runtime_core.db".to_string()
            )
        );
    }
    ...
    fn test_libsql_autodetect_sets_backend_when_db_exists() {
        ...
        let dir = tempdir().unwrap();
        let db_path = dir.path().join("ferrumyx_runtime_core.db");
    ...
    #[test]
    fn test_runtime_core_base_dir_default() {
        // This test must run first (or in isolation) before the LazyLock is initialized.
        // It verifies that when FERRUMYX_RUNTIME_CORE_BASE_DIR is not set, the default path is used.
        let _guard = ENV_MUTEX.lock().unwrap();
        let old_val = std::env::var("FERRUMYX_RUNTIME_CORE_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::remove_var("FERRUMYX_RUNTIME_CORE_BASE_DIR") };

        // Force re-evaluation by calling the computation function directly
        let path = compute_runtime_core_base_dir();
        let home = dirs::home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
        assert_eq!(path, home.join(".ferrumyx_runtime_core"));

        if let Some(val) = old_val {
            // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
            unsafe { std::env::set_var("FERRUMYX_RUNTIME_CORE_BASE_DIR", val) };
        }
    }

    #[test]
    fn test_runtime_core_base_dir_env_override() {
        // This test verifies that when FERRUMYX_RUNTIME_CORE_BASE_DIR is set,
        // the custom path is used. Must run before LazyLock is initialized.
        let _guard = ENV_MUTEX.lock().unwrap();
        let old_val = std::env::var("FERRUMYX_RUNTIME_CORE_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("FERRUMYX_RUNTIME_CORE_BASE_DIR", "/custom/ferrumyx_runtime_core/path") };

        // Force re-evaluation by calling the computation function directly
        let path = compute_runtime_core_base_dir();
        assert_eq!(path, std::path::PathBuf::from("/custom/ferrumyx_runtime_core/path"));
    ...
    fn test_compute_base_dir_env_path_join() {
        // Verifies that runtime_core_env_path correctly joins .env to the base dir.
        // Uses compute_runtime_core_base_dir directly to avoid LazyLock caching.
        let _guard = ENV_MUTEX.lock().unwrap();
        let old_val = std::env::var("FERRUMYX_RUNTIME_CORE_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("FERRUMYX_RUNTIME_CORE_BASE_DIR", "/my/custom/dir") };

        // Test the path construction logic directly
        let base_path = compute_runtime_core_base_dir();
    ...
    #[test]
    fn test_runtime_core_base_dir_empty_env() {
        // Verifies that empty FERRUMYX_RUNTIME_CORE_BASE_DIR falls back to default.
        let _guard = ENV_MUTEX.lock().unwrap();
        let old_val = std::env::var("FERRUMYX_RUNTIME_CORE_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("FERRUMYX_RUNTIME_CORE_BASE_DIR", "") };
    ...
    #[test]
    fn test_runtime_core_base_dir_special_chars() {
        // Verifies that paths with special characters are handled correctly.
        let _guard = ENV_MUTEX.lock().unwrap();
        let old_val = std::env::var("FERRUMYX_RUNTIME_CORE_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("FERRUMYX_RUNTIME_CORE_BASE_DIR", "/tmp/test_with-special.chars") };
    ...
    fn test_pid_lock_acquire_and_drop() {
        let dir = tempdir().unwrap();
        let pid_path = dir.path().join("ferrumyx_runtime_core.pid");
    ...
    fn test_pid_lock_creates_parent_dirs() {
        let dir = tempdir().unwrap();
        let pid_path = dir.path().join("nested").join("deep").join("ferrumyx_runtime_core.pid");
    ...
    fn test_pid_lock_child_helper_holds_lock() {
        if std::env::var("FERRUMYX_RUNTIME_CORE_PID_LOCK_CHILD").ok().as_deref() != Some("1") {
            return;
        }

        let pid_path = PathBuf::from(
            std::env::var("FERRUMYX_RUNTIME_CORE_PID_LOCK_PATH").expect("FERRUMYX_RUNTIME_CORE_PID_LOCK_PATH missing"),
        );
        let hold_ms = std::env::var("FERRUMYX_RUNTIME_CORE_PID_LOCK_HOLD_MS")
            .ok()
    ...
    fn test_pid_lock_rejects_lock_held_by_other_process() {
        ...
        let mut child = Command::new(current_exe)
            ...
            .env("FERRUMYX_RUNTIME_CORE_PID_LOCK_CHILD", "1")
            .env("FERRUMYX_RUNTIME_CORE_PID_LOCK_PATH", pid_path.display().to_string())
            .env("FERRUMYX_RUNTIME_CORE_PID_LOCK_HOLD_MS", "3000")
            .spawn()
            .unwrap();
