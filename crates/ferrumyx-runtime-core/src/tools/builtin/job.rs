        ...
        for (secret_name, env_var_value) in creds_obj {
            ...
            if !exists {
                return Err(ToolError::ExecutionFailed(format!(
                    "secret '{}' not found. Store it first via 'ferrumyx-runtime-core tool auth' or the web UI.",
                    secret_name
                )));
            }
...
fn projects_base() -> PathBuf {
    runtime_core_base_dir().join("projects")
}

/// Resolve the project directory, creating it if it doesn't exist.
///
/// Auto-creates `~/.ferrumyx_runtime_core/projects/{project_id}/` so every sandbox job has a
...
/// it is validated to fall within `~/.ferrumyx_runtime_core/projects/` after canonicalization.
            ...
            serde_json::json!({
                ...
                "properties": {
                    ...
                    "mode": {
                        ...
                        "description": "Execution mode. 'worker' (default) uses the Ferrumyx Runtime Core sub-agent. \
                    ...
                    "project_dir": {
                        ...
                        "description": "Path to an existing project directory to mount into the container. \
                                        Must be under ~/.ferrumyx_runtime_core/projects/. If omitted, a fresh directory is created."
                    },
                    "credentials": {
                        "type": "object",
                        "description": "Map of secret names to env var names. Each secret must exist in the \
                                        secrets store (via 'ferrumyx-runtime-core tool auth' or web UI). Example: \
