...
mod tests {
    ...
    fn test_save_to_accepts_simple_tmp_path() {
        let path = validate_save_to_path("/tmp/test_runtime_core_photo.jpg").unwrap();
    ...
    fn test_save_to_accepts_nested_tmp_path() {
        let path = validate_save_to_path("/tmp/runtime_core_test_subdir/nested/file.png").unwrap();
        assert!(path.starts_with("/tmp"));
        let _ = std::fs::remove_dir_all("/tmp/runtime_core_test_subdir");
    }
