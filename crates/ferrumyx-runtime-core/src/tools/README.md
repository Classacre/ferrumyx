...
5. Install with `ferrumyx-runtime-core tool install path/to/tool.wasm`
...
When running `ferrumyx-runtime-core tool auth <tool>`:
...
Both are first-class in the extension system (`ferrumyx-runtime-core tool install` handles both), but they have different strengths.

**WASM Tools (Ferrumyx Runtime Core native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, Ferrumyx Runtime Core can't prevent leaks
