...
//! loader.load_from_dir(Path::new("~/.ferrumyx_runtime_core/tools/")).await?;
//! ```
//!
//! # Dev Mode
//!
...
///
/// Checks (in order):
/// 1. `FERRUMYX_RUNTIME_CORE_TOOLS_SRC` env var
/// 2. `<CARGO_MANIFEST_DIR>/tools-src/` (dev builds)
fn tools_src_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("FERRUMYX_RUNTIME_CORE_TOOLS_SRC") {
...
/// needing to install them to `~/.ferrumyx_runtime_core/tools/` first. Build artifacts
/// that are newer than installed copies take priority.
///
/// Set `FERRUMYX_RUNTIME_CORE_TOOLS_SRC` env var to override the source directory.
