// Ferrumyx Runtime Core Web Gateway - Client
...
function authenticate() {
  ...
  apiFetch('/api/chat/threads')
    .then(() => {
      sessionStorage.setItem('ferrumyx-runtime-core_token', token);
    ...
    .catch(() => {
      sessionStorage.removeItem('ferrumyx-runtime-core_token');
...
(function autoAuth() {
  ...
  const saved = sessionStorage.getItem('ferrumyx-runtime-core_token');
  if (saved) {
...
function fetchGatewayStatus() {
  apiFetch('/api/gateway/status').then(function(data) {
    ...
    // Version
    if (data.version) {
      html += '<div class="gw-section-label">Ferrumyx Runtime Core v' + escapeHtml(data.version) + '</div>';
