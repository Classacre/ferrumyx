...
///
/// Used on hosted instances where `FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL` points to
...
async fn oauth_callback_handler(
    ...
    let state_param = match params.get("state") {
        ...
        _ => return oauth_error_page("Ferrumyx Runtime Core"),
    };
    ...
    let ext_mgr = match state.extension_manager.as_ref() {
        Some(mgr) => mgr,
        None => return oauth_error_page("Ferrumyx Runtime Core"),
    };
    ...
    let flow = match flow {
        ...
        None => {
            ...
            return oauth_error_page("Ferrumyx Runtime Core");
        }
    };
    ...
    let exchange_proxy_url = std::env::var("FERRUMYX_RUNTIME_CORE_OAUTH_EXCHANGE_URL").ok();
                ...
                crate::extensions::ExtensionSource::WasmBuildable { .. } => {
                    format!(
                        "'{}' requires building from source. \
                         Run `ferrumyx-runtime-core registry install {}` from the CLI.",
                        req.name, req.name
                    )
                }
                _ => format!(
...
/// Shared logic: resolve the file inside `~/.ferrumyx_runtime_core/projects/{project_id}/`,
...
async fn serve_project_file(project_id: &str, path: &str) -> axum::response::Response {
    ...
    let base = runtime_core_base_dir().join("projects").join(project_id);
...
async fn gateway_status_handler(
    ...
    let restart_enabled = std::env::var("FERRUMYX_RUNTIME_CORE_IN_DOCKER")
