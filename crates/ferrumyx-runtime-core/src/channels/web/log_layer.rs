...
/// (e.g. `ferrumyx_runtime_core=debug`) without restarting the process.
...
impl LogLevelHandle {
    ...
    /// Change the `ferrumyx_runtime_core=<level>` directive at runtime.
    ///
    ...
    pub fn set_level(&self, level: &str) -> Result<(), String> {
        ...
        let filter_str = if self.base_filter.is_empty() {
            format!("ferrumyx_runtime_core={}", level)
        } else {
            format!("ferrumyx_runtime_core={},{}", level, self.base_filter)
        };
    ...
    /// Returns the current ferrumyx-runtime-core log level (e.g. "info", "debug").
...
pub fn init_tracing(log_broadcaster: Arc<LogBroadcaster>) -> Arc<LogLevelHandle> {
    let raw_filter =
        std::env::var("RUST_LOG").unwrap_or_else(|_| "ferrumyx_runtime_core=info,tower_http=warn".to_string());

    // Split into the ferrumyx-runtime-core directive and "everything else" (base_filter).
    let mut runtime_core_level = String::from("info");
    let mut base_parts: Vec<&str> = Vec::new();

    for part in raw_filter.split(',') {
        let trimmed = part.trim();
        if trimmed.starts_with("ferrumyx_runtime_core=") {
            if let Some(lvl) = trimmed.strip_prefix("ferrumyx_runtime_core=") {
                runtime_core_level = lvl.to_string();
            }
...
///   INFO ferrumyx_runtime_core::agent: Request completed url="http://..." status=200
///
...
mod tests {
    ...
    fn test_log_broadcaster_subscribe() {
        ...
        broadcaster.send(LogEntry {
            ...
            target: "ferrumyx_runtime_core::test".to_string(),
    ...
    fn test_log_entry_serialization() {
        let entry = LogEntry {
            ...
            target: "ferrumyx_runtime_core::agent".to_string(),
