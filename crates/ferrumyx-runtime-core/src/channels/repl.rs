...
fn print_help() {
    ...
    println!();
    println!("  {h}Ferrumyx Runtime Core REPL{r}");
    println!();
...
/// Get the history file path (~/.ferrumyx_runtime_core/history).
fn history_path() -> std::path::PathBuf {
    runtime_core_base_dir().join("history")
}
    ...
    async fn start(&self) -> Result<MessageStream, ChannelError> {
        ...
        std::thread::spawn(move || {
            ...
            if !suppress_banner.load(Ordering::Relaxed) {
                println!("\x1b[1mFerrumyx Runtime Core\x1b[0m  /help for commands, /quit to exit");
                println!();
            }
