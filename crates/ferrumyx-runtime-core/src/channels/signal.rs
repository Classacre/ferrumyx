...
impl SignalChannel {
    ...
    fn handle_pairing_request(&self, sender: &str, source_name: Option<&str>) -> Result<bool, ()> {
        ...
        match store.upsert_request("signal", sender, Some(meta)) {
            Ok(result) => {
                ...
                if result.created {
                    let message = format!(
                        "To pair with this bot, run: `ferrumyx-runtime-core pairing approve signal {}`",
                        result.code
                    );
    ...
    /// - All paths are within ~/.ferrumyx_runtime_core/ sandbox
...
mod tests {
    ...
    fn validate_attachment_paths_accepts_normal_paths() {
        ...
        let base_dir = crate::bootstrap::runtime_core_base_dir();
