...
fn read_attachments(paths: &[String]) -> Result<Vec<wit_channel::Attachment>, String> {
    ...
    let home_base = dirs::home_dir()
        .map(|h| h.join(".ferrumyx_runtime_core"))
        .unwrap_or_default();

    for path in paths {
        // Validate paths are under /tmp/ or ~/.ferrumyx_runtime_core/ to prevent arbitrary file reads
        ...
        let validated = validated.map_err(|e| {
            format!(
                "Invalid attachment path '{}': must be under /tmp/ or ~/.ferrumyx_runtime_core/: {}",
                path, e
            )
        })?;
...
mod tests {
    ...
    fn test_mime_from_extension() {
        ...
        assert_eq!(
            mime_from_extension("/home/user/.ferrumyx_runtime_core/screenshot.png"),
            "image/png"
        );
    }
}
