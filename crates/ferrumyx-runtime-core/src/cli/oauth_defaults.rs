...
//!
//! - **Compile time**: Set FERRUMYX_RUNTIME_CORE_GOOGLE_CLIENT_ID / FERRUMYX_RUNTIME_CORE_GOOGLE_CLIENT_SECRET
...
const GOOGLE_CLIENT_ID: &str = match option_env!("FERRUMYX_RUNTIME_CORE_GOOGLE_CLIENT_ID") {
    Some(v) => v,
    None => "YOUR_GOOGLE_OAUTH_CLIENT_ID",
};
const GOOGLE_CLIENT_SECRET: &str = match option_env!("FERRUMYX_RUNTIME_CORE_GOOGLE_CLIENT_SECRET") {
    Some(v) => v,
...
///
/// Checks `FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL` env var first (useful for remote/VPS
...
pub fn callback_url() -> String {
    std::env::var("FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL")
        .ok()
        .filter(|v| !v.is_empty())
...
<title>Ferrumyx Runtime Core - {heading}</title>
<style>
...
<body>
  <div class="card">
    ...
    <h1>{heading}</h1>
    <p>{subtitle}</p>
    <div class="brand">Ferrumyx Runtime Core</div>
  </div>
</body>
</html>"#,
        heading = heading,
...
///
/// This is the case when `FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL` is set to a non-loopback
...
///
/// Returns the nonce unchanged when `FERRUMYX_RUNTIME_CORE_INSTANCE_NAME` is not set
/// (local/non-platform mode).
pub fn build_platform_state(nonce: &str) -> String {
    let instance = std::env::var("FERRUMYX_RUNTIME_CORE_INSTANCE_NAME")
...
mod tests {
    ...
    fn test_callback_host_env_override() {
        ...
        let original_url = std::env::var("FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("OAUTH_CALLBACK_HOST", "203.0.113.10");
            std::env::remove_var("FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL");
        }
        ...
        unsafe {
            ...
            if let Some(val) = original_url {
                std::env::set_var("FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL", val);
            }
        }
    }
    ...
    fn test_callback_url_env_override() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        let original = std::env::var("FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var(
                "FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL",
    ...
    fn test_landing_html_success_contains_key_elements() {
        ...
        assert!(html.contains("Ferrumyx Runtime Core"));
    ...
    fn test_use_gateway_callback_false_for_localhost() {
        ...
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL", "http://127.0.0.1:3001");
        }
        assert!(!crate::cli::oauth_defaults::use_gateway_callback());
        unsafe {
            if let Some(val) = original {
    ...
    fn test_use_gateway_callback_false_for_empty() {
        ...
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL", "");
        }
        assert!(!crate::cli::oauth_defaults::use_gateway_callback());
        unsafe {
            if let Some(val) = original {
    ...
    fn test_build_platform_state_with_instance() {
        ...
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        let original = std::env::var("FERRUMYX_RUNTIME_CORE_INSTANCE_NAME").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("FERRUMYX_RUNTIME_CORE_INSTANCE_NAME", "kind-deer");
        }
        assert_eq!(build_platform_state("abc123"), "kind-deer:abc123");
        unsafe {
            if let Some(val) = original {
                std::env::set_var("FERRUMYX_RUNTIME_CORE_INSTANCE_NAME", val);
            } else {
                std::env::remove_var("FERRUMYX_RUNTIME_CORE_INSTANCE_NAME");
            }
        }
    }
    ...
    fn test_build_platform_state_with_openclaw_instance() {
        ...
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        let original_ic = std::env::var("FERRUMYX_RUNTIME_CORE_INSTANCE_NAME").ok();
        let original_oc = std::env::var("OPENCLAW_INSTANCE_NAME").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::remove_var("FERRUMYX_RUNTIME_CORE_INSTANCE_NAME");
