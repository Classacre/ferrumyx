//! `ferrumyx-runtime-core doctor` - active health diagnostics.
//!
...
pub async fn run_doctor_command() -> anyhow::Result<()> {
    println!("Ferrumyx Runtime Core Doctor");
    println!("===============\n");
...
async fn check_nearai_session() -> CheckResult {
    ...
    if !session_path.exists() {
        ...
        return CheckResult::Fail(format!(
            "session file not found at {}. Run `ferrumyx-runtime-core onboard`",
...
mod tests {
    ...
    fn check_binary_skips_nonexistent() {
        match check_binary("__runtime_core_nonexistent_binary__", &["--version"]) {
            CheckResult::Skip(_) => {}
            other => panic!(
