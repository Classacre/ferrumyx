...
pub enum ToolCommand {
    ...
    Install {
        ...
        /// Target directory for installation (default: ~/.ferrumyx_runtime_core/tools/)
        #[arg(short, long)]
    ...
    List {
        /// Directory to list tools from (default: ~/.ferrumyx_runtime_core/tools/)
        #[arg(short, long)]
        dir: Option<PathBuf>,
    ...
    Remove {
        ...
        /// Directory to remove tool from (default: ~/.ferrumyx_runtime_core/tools/)
        #[arg(short, long)]
        dir: Option<PathBuf>,
    },
    ...
    Info {
        ...
        /// Directory to look for tool (default: ~/.ferrumyx_runtime_core/tools/)
        #[arg(short, long)]
        dir: Option<PathBuf>,
    },
...
async fn list_tools(dir: Option<PathBuf>, verbose: bool) -> anyhow::Result<()> {
    ...
    if !tools_dir.exists() {
        ...
        println!("Install a tool with: ferrumyx-runtime-core tool install <path>");
        return Ok(());
    }
...
async fn init_secrets_store() -> anyhow::Result<Arc<dyn SecretsStore + Send + Sync>> {
    ...
    let master_key = config.secrets.master_key().ok_or_else(|| {
        anyhow::anyhow!(
            "SECRETS_MASTER_KEY not set. Run 'ferrumyx-runtime-core onboard' first or set it in .env"
        )
    })?;
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "OAuth client_id not configured.\n\
                 Set {} env var, or build with FERRUMYX_RUNTIME_CORE_GOOGLE_CLIENT_ID.",
    ...
    let setup = caps.setup.ok_or_else(|| {
        anyhow::anyhow!(
            "Tool '{}' has no setup configuration.\n\
             ...
             Try 'ferrumyx-runtime-core tool auth {}' for OAuth-based authentication.",
            name,
            name
        )
    })?;
...
mod tests {
    ...
    fn test_default_tools_dir() {
        ...
        assert!(dir.to_string_lossy().contains(".ferrumyx_runtime_core"));
