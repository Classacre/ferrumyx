...
async fn add_server(args: McpAddArgs) -> anyhow::Result<()> {
    ...
    if requires_auth {
        println!();
        println!("  Run 'ferrumyx-runtime-core mcp auth {}' to authenticate.", name);
    }
...
async fn list_servers(verbose: bool) -> anyhow::Result<()> {
    ...
    if servers.servers.is_empty() {
        ...
        println!("    ferrumyx-runtime-core mcp add <name> <url> [--client-id <id>]");
        println!();
        return Ok(());
    }
...
async fn auth_server(name: String, user_id: String) -> anyhow::Result<()> {
    ...
    match authorize_mcp_server(&server, &secrets, &user_id).await {
        ...
        Err(crate::tools::mcp::auth::AuthError::NotSupported) => {
            ...
            println!();
            println!("    ferrumyx-runtime-core mcp remove {}", name);
            println!(
                "    ferrumyx-runtime-core mcp add {} {} --client-id YOUR_CLIENT_ID",
                name, server.url
            );
            println!();
        }
...
async fn test_server(name: String, user_id: String) -> anyhow::Result<()> {
    ...
    let client = if has_tokens {
        ...
        println!();
        println!(
            "  ✗ Not authenticated. Run 'ferrumyx-runtime-core mcp auth {}' first.",
            name
        );
        println!();
        return Ok(());
        ...
        Err(e) => {
            ...
            if err_str.contains("401") || err_str.contains("requires authentication") {
                if has_tokens {
                    ...
                    println!("    ferrumyx-runtime-core mcp auth {}", name);
                } else {
