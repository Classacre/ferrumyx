...
This document is the authoritative specification for Ferrumyx Runtime Core's onboarding
...
```
ferrumyx-runtime-core onboard [--skip-auth] [--channels-only]
```

Explicit invocation. Loads `.env` files, runs the wizard, exits.

```
ferrumyx-runtime-core          (first run, no database configured)
```

Auto-detection via `check_onboard_needed()` in `main.rs`. Skips onboarding
when `ONBOARD_COMPLETED` env var is set (written to `~/.ferrumyx-runtime-core/.env` by
the wizard). Otherwise triggers when no database is configured:
- `DATABASE_URL` env var is set
- `LIBSQL_PATH` env var is set
- `~/.ferrumyx-runtime-core/ferrumyx-runtime-core.db` exists on disk
...
3. If Command::Run or no command:
   a. Load .env files (dotenvy::dotenv() then load_ferrumyx-runtime-core_env())
...
`DATABASE_BACKEND` live in `~/.ferrumyx-runtime-core/.env`.
...
1. Offer local path (default: `~/.ferrumyx-runtime-core/ferrumyx-runtime-core.db`)
...
2. "Allow ferrumyx-runtime-core to access this keychain item" (per-app authorization)
...
- **Never probe the keychain in read-only commands** (e.g., `ferrumyx-runtime-core status`).
...
- **NEAR AI Chat** path: session token saved to `~/.ferrumyx-runtime-core/session.json`.
  Hosting providers can set `NEARAI_SESSION_TOKEN` env var directly (takes
  precedence over file-based tokens).
- **NEAR AI Cloud** path: `NEARAI_API_KEY` saved to `~/.ferrumyx-runtime-core/.env`
...
6b. Discover WASM channels from ~/.ferrumyx-runtime-core/channels/
...
1. Already installed in `~/.ferrumyx-runtime-core/channels/`
...
4. Discover already-installed tools in `~/.ferrumyx-runtime-core/tools/`
...
**Layer 1: `~/.ferrumyx-runtime-core/.env`** (bootstrap vars)
...
LIBSQL_PATH="/Users/name/.ferrumyx-runtime-core/ferrumyx-runtime-core.db"
...
DATABASE_URL="postgres://user:pass@localhost/ferrumyx-runtime-core"
LLM_BACKEND="nearai"
```
...
1. Writes bootstrap vars to `~/.ferrumyx-runtime-core/.env` via `write_bootstrap_env()`
...
Bootstrap vars written to `~/.ferrumyx-runtime-core/.env`:
...
pub struct Settings {
    ...
    // Advanced (not in wizard, set via `ferrumyx-runtime-core config set`)
...
- Service name: `"ferrumyx-runtime-core"`, account: `"master_key"`
...
1. **NEAR AI Cloud API key (option 4 in auth menu):** Get an API key
   ...
   local listener is needed. The key is saved to `~/.ferrumyx-runtime-core/.env`
   and the encrypted secrets store. Uses the OpenAI-compatible
   ChatCompletions API mode.

2. **Custom callback URL:** Set `FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL` to a
   publicly accessible URL (e.g., via SSH tunnel or reverse proxy) that
   forwards to port 9876 on the server:
   ```bash
   export FERRUMYX_RUNTIME_CORE_OAUTH_CALLBACK_URL=https://myserver.example.com:9876
   ```
...
6. Run the full shipping checklist:
   ...
   ```
7. Test a fresh onboarding: `rm -rf ~/.ferrumyx-runtime-core && cargo run`
