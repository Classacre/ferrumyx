...
use crate::bootstrap::runtime_core_base_dir;
...
/// Interactive setup wizard for Ferrumyx Runtime Core.
...
impl SetupWizard {
    ...
    pub async fn run(&mut self) -> Result<(), SetupError> {
        print_header("Ferrumyx Runtime Core Setup Wizard");
    ...
    async fn reconnect_existing_db(&mut self) -> Result<(), SetupError> {
        ...
        Err(SetupError::Database(
            "No database configured. Run full setup first (ferrumyx-runtime-core onboard).".to_string(),
        ))
    }
    ...
    async fn reconnect_postgres(&mut self) -> Result<(), SetupError> {
        let url = std::env::var("DATABASE_URL").map_err(|_| {
            SetupError::Database(
                "DATABASE_URL not set. Run full setup first (ferrumyx-runtime-core onboard).".to_string(),
            )
        })?;
    ...
    async fn step_database_libsql(&mut self) -> Result<(), SetupError> {
        ...
        println!();
        print_info("Ferrumyx Runtime Core uses an embedded SQLite database (libSQL).");
    ...
    async fn test_database_connection_postgres(&mut self, url: &str) -> Result<(), SetupError> {
        ...
        if major_version < MIN_PG_MAJOR_VERSION {
            return Err(SetupError::Database(format!(
                "PostgreSQL {} detected. Ferrumyx Runtime Core requires PostgreSQL {} or later for pgvector support.\n\
        ...
        if pgvector_row.is_none() {
            return Err(SetupError::Database(format!(
                "pgvector extension not found on your PostgreSQL server.\n\n\
                 ...
                 Then restart PostgreSQL and re-run: ferrumyx-runtime-core onboard",
                major_version
            )));
        }
    ...
    async fn step_security(&mut self) -> Result<(), SetupError> {
        ...
        match choice {
            ...
            1 => {
                ...
                // Store hex for write_bootstrap_env to persist to ~/.ferrumyx_runtime_core/.env.
                self.settings.secrets_master_key_hex = Some(key_hex.clone());

                println!();
                print_info("Master key generated and will be saved to ~/.ferrumyx_runtime_core/.env");
                println!();
    ...
    async fn step_channels(&mut self) -> Result<(), SetupError> {
        ...
        let channels_dir = runtime_core_base_dir().join("channels");
    ...
    async fn step_extensions(&mut self) -> Result<(), SetupError> {
        let catalog = match load_registry_catalog() {
            ...
            None => {
                ...
                print_info("Install tools manually with: ferrumyx-runtime-core tool install <path>");
                return Ok(());
            }
        };
        ...
        print_info("  ferrumyx-runtime-core registry install <name>");
        println!();

        // Check which tools are already installed
        let tools_dir = runtime_core_base_dir().join("tools");
        ...
        let installer = crate::registry::installer::RegistryInstaller::new(
            ...
            tools_dir.clone(),
            runtime_core_base_dir().join("channels"),
        );
            ...
            match installer.install_with_source_fallback(tool, false).await {
                Ok(outcome) => {
                        ...
                        && auth.method.is_some()
                    {
                        ...
                        let hint = format!("  {} - ferrumyx-runtime-core tool auth {}", provider, tool.name);
                        if !auth_needed
                            .iter()
    ...
    async fn step_docker_sandbox(&mut self) -> Result<(), SetupError> {
        print_info("Ferrumyx Runtime Core can execute code, run builds, and use tools inside Docker");
    ...
    /// Write bootstrap environment variables to `~/.ferrumyx_runtime_core/.env`.
    ///
    ...
    /// Save settings to the database and `~/.ferrumyx_runtime_core/.env`, then print summary.
    async fn save_and_summarize(&mut self) -> Result<(), SetupError> {
        ...
        println!("  ferrumyx_runtime_core");
        println!();
        println!("To change settings later:");
        println!("  ferrumyx-runtime-core config set <setting> <value>");
        println!("  ferrumyx-runtime-core onboard");
        println!();
...
async fn install_selected_registry_channels(
    ...
    for name in selected_channels {
        ...
        let installer = crate::registry::installer::RegistryInstaller::new(
            repo_root.clone(),
            runtime_core_base_dir().join("tools"),
...
mod tests {
    ...
    async fn test_discover_wasm_channels_nonexistent_dir() {
        let channels = discover_wasm_channels(
            &std::env::temp_dir().join("runtime_core_nonexistent_dir_abcxyz123"),
        )
        .await;
        assert!(channels.is_empty());
    }
