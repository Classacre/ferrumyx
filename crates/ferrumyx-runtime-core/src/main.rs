//! Ferrumyx Runtime Core - Main entry point.
...
use ferrumyx_runtime_core::setup::{SetupConfig, SetupWizard};
...
fn main() -> anyhow::Result<()> {
    ...
    ferrumyx_runtime_core::bootstrap::load_runtime_core_env();
...
async fn async_main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Some(Command::Config(config_cmd)) => {
            init_cli_tracing();
            return ferrumyx_runtime_core::cli::run_config_command(config_cmd.clone()).await;
        }
        Some(Command::Registry(registry_cmd)) => {
            init_cli_tracing();
            return ferrumyx_runtime_core::cli::run_registry_command(registry_cmd.clone()).await;
        }
        ...
        Some(Command::Doctor) => {
            init_cli_tracing();
            return ferrumyx_runtime_core::cli::run_doctor_command().await;
        }
        Some(Command::Status) => {
            init_cli_tracing();
    ...
    let _pid_lock = match ferrumyx_runtime_core::bootstrap::PidLock::acquire() {
        Ok(lock) => Some(lock),
        Err(ferrumyx_runtime_core::bootstrap::PidLockError::AlreadyRunning { pid }) => {
            anyhow::bail!(
                "Another Ferrumyx Runtime Core instance is already running (PID {}). \
                 If this is incorrect, remove the stale PID file: {}",
                pid,
                ferrumyx_runtime_core::bootstrap::pid_lock_path().display()
            );
        }
        Err(e) => {
    ...
    let config = match Config::from_env_with_toml(toml_path).await {
        Ok(c) => c,
        Err(ferrumyx_runtime_core::error::ConfigError::MissingRequired { key, hint }) => {
            anyhow::bail!(
                "Configuration error: Missing required setting '{}'. {}. \
                 Run 'ferrumyx-runtime-core onboard' to configure, or set the required environment variables.",
                key,
                hint
            );
        }
    ...
    let log_level_handle =
        ferrumyx_runtime_core::channels::web::log_layer::init_tracing(Arc::clone(&log_broadcaster));

    tracing::info!("Starting Ferrumyx Runtime Core...");
    ...
    let docker_status = if config.sandbox.enabled {
        let detection = ferrumyx_runtime_core::sandbox::check_docker().await;
        match detection.status {
            ferrumyx_runtime_core::sandbox::DockerStatus::Available => {
                tracing::info!("Docker is available");
            }
            ferrumyx_runtime_core::sandbox::DockerStatus::NotInstalled => {
                tracing::warn!(
                    "Docker is not installed -- sandbox disabled for this session. {}",
                    detection.platform.install_hint()
                );
            }
            ferrumyx_runtime_core::sandbox::DockerStatus::NotRunning => {
                tracing::warn!(
                    "Docker is installed but not running -- sandbox disabled for this session. {}",
                    detection.platform.start_hint()
                );
            }
            ferrumyx_runtime_core::sandbox::DockerStatus::Disabled => {}
        }
        detection.status
    } else {
        ferrumyx_runtime_core::sandbox::DockerStatus::Disabled
    ...
    let job_event_tx: Option<
        tokio::sync::broadcast::Sender<(uuid::Uuid, ferrumyx_runtime_core::channels::web::types::SseEvent)>,
    ...
    let prompt_queue = Arc::new(tokio::sync::Mutex::new(std::collections::HashMap::<
        uuid::Uuid,
        std::collections::VecDeque<ferrumyx_runtime_core::orchestrator::api::PendingPrompt>,
    >::new()));
    ...
    let container_job_manager: Option<Arc<ContainerJobManager>> =
        if config.sandbox.enabled && docker_status.is_ok() {
            ...
            let job_config = ContainerJobConfig {
                ...
                claude_code_oauth_token: ferrumyx_runtime_core::config::ClaudeCodeConfig::extract_oauth_token(),
    ...
    let session_manager =
        Arc::new(ferrumyx_runtime_core::agent::SessionManager::new().with_hooks(components.hooks.clone()));
    ...
    let scheduler_slot: ferrumyx_runtime_core::tools::builtin::SchedulerSlot =
    ...
    let mut sse_sender: Option<
        tokio::sync::broadcast::Sender<ferrumyx_runtime_core::channels::web::types::SseEvent>,
    > = None;
    let mut routine_engine_slot: Option<ferrumyx_runtime_core::channels::web::server::RoutineEngineSlot> = None;
    ...
    if config.channels.cli.enabled && cli.message.is_none() {
        let boot_info = ferrumyx_runtime_core::boot_screen::BootInfo {
        ...
        ferrumyx_runtime_core::boot_screen::print_boot_screen(&boot_info);
    }
    ...
    let deps = AgentDeps {
        ...
        transcription: config
            .transcription
            .create_provider()
            .map(|p| Arc::new(ferrumyx_runtime_core::transcription::TranscriptionMiddleware::new(p))),
        document_extraction: Some(Arc::new(
            ferrumyx_runtime_core::document_extraction::DocumentExtractionMiddleware::new(),
        )),
    };
...
fn init_worker_tracing() {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("ferrumyx_runtime_core=info")),
        )
        .init();
}

/// Run the Memory CLI subcommand.
async fn run_memory_command(mem_cmd: &ferrumyx_runtime_core::cli::MemoryCommand) -> anyhow::Result<()> {
    ...
    let db: Arc<dyn ferrumyx_runtime_core::db::Database> = ferrumyx_runtime_core::db::connect_from_config(&config.database)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e))?;

    ferrumyx_runtime_core::cli::run_memory_command_with_db(mem_cmd.clone(), db, embeddings).await
}
...
async fn run_worker(
    ...
    let config = ferrumyx_runtime_core::worker::runtime::WorkerConfig {
        job_id,
    ...
    let runtime = ferrumyx_runtime_core::worker::WorkerRuntime::new(config)
...
async fn run_claude_bridge(
    ...
    let config = ferrumyx_runtime_core::worker::claude_bridge::ClaudeBridgeConfig {
        job_id,
        orchestrator_url: orchestrator_url.to_string(),
        max_turns,
        model: model.to_string(),
        timeout: std::time::Duration::from_secs(1800),
        allowed_tools: ferrumyx_runtime_core::config::ClaudeCodeConfig::from_env().allowed_tools,
    };

    let runtime = ferrumyx_runtime_core::worker::ClaudeBridgeRuntime::new(config)
...
async fn start_tunnel(
    mut config: ferrumyx_runtime_core::config::Config,
) -> (
    ferrumyx_runtime_core::config::Config,
    Option<Box<dyn ferrumyx_runtime_core::tunnel::Tunnel>>,
) {
    ...
    match ferrumyx_runtime_core::tunnel::create_tunnel(provider_config) {
        Ok(Some(tunnel)) => {
            tracing::info!(
                "Starting {} tunnel on {}:{}...",
                tunnel.name(),
...
struct WasmChannelSetup {
    channels: Vec<(String, Box<dyn ferrumyx_runtime_core::channels::Channel>)>,
...
async fn setup_wasm_channels(
    config: &ferrumyx_runtime_core::config::Config,
    secrets_store: &Option<Arc<dyn SecretsStore + Send + Sync>>,
    extension_manager: Option<&Arc<ferrumyx_runtime_core::extensions::ExtensionManager>>,
    database: Option<&Arc<dyn ferrumyx_runtime_core::db::Database>>,
    ...
    let settings_store: Option<Arc<dyn ferrumyx_runtime_core::db::SettingsStore>> =
        database.map(|db| Arc::clone(db) as Arc<dyn ferrumyx_runtime_core::db::SettingsStore>);
    ...
    let mut channels: Vec<(String, Box<dyn ferrumyx_runtime_core::channels::Channel>)> = Vec::new();
    let mut channel_names: Vec<String> = Vec::new();
...
fn check_onboard_needed() -> Option<&'static str> {
    let has_db = std::env::var("DATABASE_URL").is_ok()
        ...
        || ferrumyx_runtime_core::config::default_libsql_path().exists();
    ...
    if std::env::var("NEARAI_API_KEY").is_err() {
        let session_path = ferrumyx_runtime_core::llm::session::default_session_path();
...
async fn inject_channel_credentials(
    channel: &Arc<ferrumyx_runtime_core::channels::wasm::WasmChannel>,
