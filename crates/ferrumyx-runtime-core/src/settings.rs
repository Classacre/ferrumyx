...
//!
//! Stores user preferences in ~/.ferrumyx_runtime_core/settings.json.
...
fn default_agent_name() -> String {
    "ferrumyx_runtime_core".to_string()
}
...
fn default_sandbox_image() -> String {
    "ferrumyx_runtime_core-worker:latest".to_string()
}
...
impl Settings {
    ...
    /// Get the default settings file path (~/.ferrumyx_runtime_core/settings.json).
    pub fn default_path() -> std::path::PathBuf {
        runtime_core_base_dir().join("settings.json")
    }
    ...
    /// Default TOML config file path (~/.ferrumyx_runtime_core/config.toml).
    pub fn default_toml_path() -> PathBuf {
        runtime_core_base_dir().join("config.toml")
    }
    ...
    pub fn save_toml(&self, path: &std::path::Path) -> Result<(), String> {
        ...
        let content = format!(
            "# Ferrumyx Runtime Core configuration file.\n\
             #\n\
             # Priority: env var > this file > database settings > defaults.\n\
             # Uncomment and edit values to override defaults.\n\
             # Run `ferrumyx-runtime-core config init` to regenerate this file.\n\
             #\n\
             # Documentation: https://github.com/nearai/ferrumyx_runtime_core\n\
             \n\
             {raw}"
        );
...
mod tests {
    ...
    fn test_get_setting() {
        ...
        assert_eq!(settings.get("agent.name"), Some("ferrumyx_runtime_core".to_string()));
        assert_eq!(
    ...
    fn test_reset_setting() {
        ...
        assert_eq!(settings.agent.name, "ferrumyx_runtime_core");
    }
    ...
    fn toml_header_comment_present() {
        ...
        assert!(content.starts_with("# Ferrumyx Runtime Core configuration file."));
    ...
    #[test]
    fn default_toml_path_under_runtime_core() {
        let path = Settings::default_toml_path();
        assert!(path.to_string_lossy().contains(".ferrumyx_runtime_core"));
    ...
    fn wizard_recovery_step1_overrides_stale_db() {
        ...
        let prior_run = Settings {
            ...
            database_url: Some("postgres://old-host/ferrumyx_runtime_core".to_string()),
            llm_backend: Some("anthropic".to_string()),
        ...
        let step1_settings = Settings {
            database_backend: Some("postgres".to_string()),
            database_url: Some("postgres://new-host/ferrumyx_runtime_core".to_string()),
            ..Settings::default()
        };
        ...
        assert_eq!(
            ...
            Some("postgres://new-host/ferrumyx_runtime_core".to_string()),
