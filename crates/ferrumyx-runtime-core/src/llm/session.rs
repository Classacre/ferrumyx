...
//! OAuth flow. Tokens are stored in `~/.ferrumyx_runtime_core/session.json` and refreshed
...
pub struct SessionConfig {
    ...
    /// Path to session file (e.g., ~/.ferrumyx_runtime_core/session.json).
...
/// Get the default session file path (~/.ferrumyx_runtime_core/session.json).
pub fn default_session_path() -> PathBuf {
    runtime_core_base_dir().join("session.json")
}
...
impl SessionManager {
    ...
    /// saved to `~/.ferrumyx_runtime_core/.env` for persistence across restarts.
    ...
    async fn api_key_login(&self) -> Result<(), LlmError> {
        ...
        // Persist to ~/.ferrumyx_runtime_core/.env so the key survives restarts
...
mod tests {
    ...
    fn test_session_config_default() {
        ...
        assert!(config.session_path.to_string_lossy().contains(".ferrumyx_runtime_core"));
    }
