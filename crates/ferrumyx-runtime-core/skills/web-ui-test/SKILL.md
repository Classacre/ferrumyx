...
name: web-ui-test
version: 0.1.0
description: Test the Ferrumyx Runtime Core web UI using the Claude for Chrome browser extension.
activation:
  keywords:
    - test web ui
    - test the ui
...
Use this skill when manually testing the Ferrumyx Runtime Core web gateway UI via the Claude for Chrome browser extension.

## Prerequisites

- Ferrumyx Runtime Core must be running with `GATEWAY_ENABLED=true`
...
Wait for "Agent ferrumyx-runtime-core ready and listening" in the logs before proceeding.
...
```bash
rm -rf ~/.ferrumyx-runtime-core/installed_skills/<skill-name>
```
