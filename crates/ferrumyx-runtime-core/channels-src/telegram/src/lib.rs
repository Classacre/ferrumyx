...
//! Telegram Bot API channel for Ferrumyx Runtime Core.
//!
...
fn send_photo(
    ...
    let boundary = format!("ferrumyx_runtime_core-{}", channel_host::now_millis());
...
fn send_pairing_reply(chat_id: i64, code: &str) -> Result<(), String> {
    send_message(
        chat_id,
        &format!(
            "To pair with this bot, run: `ferrumyx-runtime-core pairing approve telegram {}`",
            code
        ),
        None,
        Some("Markdown"),
