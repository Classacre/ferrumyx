...
A standalone WASM component that provides Slack integration for Ferrumyx Runtime Core. This serves as both a functional tool and a template for building custom WASM tools.
...
```bash
mkdir -p ~/.ferrumyx-runtime-core/tools
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.ferrumyx-runtime-core/tools/slack.wasm
cp slack.capabilities.json ~/.ferrumyx-runtime-core/tools/
```
...
```bash
ferrumyx-runtime-core tool install \
  --name slack \
...
```bash
ferrumyx-runtime-core secret set slack_bot_token "xoxb-your-token-here"
```
...
```json
{
  "action": "send_message",
  "channel": "#general",
  "text": "Hello from Ferrumyx Runtime Core!"
}
```
...
```bash
ferrumyx-runtime-core secret set slack_bot_token "xoxb-..."
```
