# GitHub Tool for Ferrumyx Runtime Core
...
3. Store the token:
   ...
   ```
   ferrumyx-runtime-core secret set github_token YOUR_TOKEN
   ```
...
```text
GitHub token not found in secret store. Set it with: ferrumyx-runtime-core secret set github_token <token>...
```
...
- Update the token: `ferrumyx-runtime-core secret set github_token NEW_TOKEN`.
