//! GitHub WASM Tool for Ferrumyx Runtime Core.
//!
...
//! `ferrumyx-runtime-core secret set github_token <token>`
//!
...
fn get_github_token() -> Result<String, String> {
    ...
    Err("GitHub token not found in secret store. Set it with: ferrumyx-runtime-core secret set github_token <token>. \
...
fn github_request(method: &str, path: &str, body: Option<String>) -> Result<String, String> {
    ...
    let headers = serde_json::json!({
        ...
        "User-Agent": "Ferrumyx Runtime Core-GitHub-Tool"
    });
