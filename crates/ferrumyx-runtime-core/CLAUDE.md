# Ferrumyx Runtime Core Development Guide

## Project Overview

**Ferrumyx Runtime Core** is a secure personal AI assistant that protects your data and expands its capabilities on the fly.
...
# Run E2E tests (Python/Playwright — requires a running ferrumyx-runtime-core instance)
...
├── bootstrap.rs        # Base directory resolution (~/.ferrumyx-runtime-core), early .env loading
├── settings.rs         # User settings persistence (~/.ferrumyx-runtime-core/settings.json)
...
LIBSQL_PATH=~/.ferrumyx-runtime-core/ferrumyx-runtime-core.db    # libSQL local path (default)
...
# Agent settings
AGENT_NAME=ferrumyx-runtime-core
MAX_PARALLEL_JOBS=5
...
SANDBOX_IMAGE=ferrumyx-runtime-core-worker:latest
...
|-------------|--------|-------------|
| **Trusted** | User-placed in `~/.ferrumyx-runtime-core/skills/` or workspace `skills/` | All tools available to the agent |
...
- `~/.ferrumyx-runtime-core/skills/` -- User's global skills (trusted)
- `<workspace>/skills/` -- Per-workspace skills (trusted)
- `~/.ferrumyx-runtime-core/installed_skills/` -- Registry-installed skills (installed trust)
...
Tools can be built as **WASM** (sandboxed, credential-injected, single binary) or **MCP servers** (ecosystem of pre-built servers, any language, but no sandbox). Both are first-class via `ferrumyx-runtime-core tool install`. Auth is declared in capabilities files with OAuth and manual token entry support.
...
```bash
# Verbose logging
RUST_LOG=ferrumyx-runtime-core=trace cargo run

# Just the agent module
RUST_LOG=ferrumyx-runtime-core::agent=debug cargo run

# With HTTP request logging
RUST_LOG=ferrumyx-runtime-core=debug,tower_http=debug cargo run
```
