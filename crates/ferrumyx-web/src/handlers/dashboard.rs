...
<main class="main-content">
    <div class="page-header">
        ...
        <div class="top-toolbar">
            ...
            <a href="/query" class="btn btn-primary btn-hero">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>
                New Search
            </a>
        </div>
    </div>
...
async fn load_top_targets(state: &SharedState) -> Vec<(String, String, f64)> {
    let score_repo = TargetScoreRepository::new(state.db.clone());
    let entity_repo = EntityRepository::new(state.db.clone());
    let mut rows = score_repo.list(0, 500).await.unwrap_or_default();
    rows.sort_by(|a, b| {
    ...
    let mut out = Vec::new();
    for s in rows.into_iter().take(5) {
        let raw: serde_json::Value = serde_json::from_str(&s.components_raw).unwrap_or_default();
        let mut gene = raw
            .get("gene")
            .and_then(|v| v.as_str())
            .map(|v| v.to_string());
        ...
        out.push((gene, cancer, s.confidence_adjusted_score));
    }
    out
}
