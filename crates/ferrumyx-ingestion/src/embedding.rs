        ...
        let embedder = EMBEDDER.get_or_init(|| {
            ...
            let embedder = tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async {
                    crate::embed::BiomedBertEmbedder::new(config)
                        .await
...
fn resolve_embed_use_gpu() -> bool {
    if let Ok(v) = std::env::var("FERRUMYX_EMBED_USE_GPU") {
...
pub async fn embed_pending_chunks(
    ...
    // Process in batches
    for (batch_ids, batch_texts) in ids
        .chunks(client.cfg.batch_size)
        .zip(texts.chunks(client.cfg.batch_size))
    {
        ...
        let normalized_vecs: Vec<Vec<f32>> = vecs
            .iter()
            .map(|vec| {
        ...
        let updates: Vec<(Uuid, Vec<f32>)> = batch_ids
            .iter()
            .zip(normalized_vecs.into_iter())
            .map(|(chunk_id, embedding)| (*chunk_id, embedding))
            .collect();
        match repo.bulk_update_embeddings(&updates).await {
            ...
            Err(e) => {
                warn!("Bulk embedding update failed: {e}");
            }
        }
    }
